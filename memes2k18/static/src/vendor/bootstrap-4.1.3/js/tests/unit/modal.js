$(function () {
  ...
  QUnit.test('should ignore other inline css when trying to restore body padding after closing', function (assert) {
    assert.expect(2)
    var done = assert.async()
    var $body = $(document.body)
    var $style = $('<style>body { padding-right: 42px; }</style>').appendTo('head')
    ...
    $('<div id="modal-test"/>')
      .on('hidden.bs.modal', function () {
        ...
        assert.strictEqual($body[0].style.color, 'red', 'body still has other inline css set')
        $body.removeAttr('style')
        $style.remove()
        done()
      })
